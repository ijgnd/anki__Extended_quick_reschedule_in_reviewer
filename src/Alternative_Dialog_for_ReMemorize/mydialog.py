...
class MultiPrompt(QDialog):
    ...
    def __init__(self, co):
        ...
        self.qrs.lineEdit.textChanged.connect(self.checkText)  # stackoverflow.com/q/22520747
        self.qrs.lineEdit.installEventFilter(self)  # stackoverflow.com/questions/47960368
        tiptext = "neg=keep Ivl/change only the due date (=future intervals are unaffected)"
        self.qrs.lineEdit.setToolTip(tiptext)

        self.qrs.tb_up.setArrowType(Qt.UpArrow)
        self.qrs.tb_up.clicked.connect(lambda _: self.on_arrows(1))
        self.qrs.tb_up.setToolTip("%s" % str(self.co['one_more_day']))

        self.qrs.tb_down.setArrowType(Qt.DownArrow)
        self.qrs.tb_down.clicked.connect(lambda _: self.on_arrows(-1))
        self.qrs.tb_down.setToolTip("%s" % str(self.co['one_less_day']))

        self.qrs.pbc_clear.clicked.connect(lambda: self.qrs.lineEdit.setText(""))
        ...
        self.qrs.pb_u_clear.clicked.connect(lambda: self.qrs.lineEdit.setText(""))
        ...
        if self.co['add_quick_buttons_to_dialog']:
            for v in self.co['quick_buttons']:
                ...
                k.setText(v['label'])
                k.setToolTip("%s" % str(v['key']))
                k.setContextMenuPolicy(Qt.CustomContextMenu)
                k.customContextMenuRequested.connect(lambda _, a=v['ivl']:
                                                     self.set_and_accept(-1 * a))
                k.clicked.connect(lambda _, a=v['ivl']: self.set_and_accept(a))
                self.qrs.vL_custom.addWidget(k)
            qf = QPushButton()
            qf.setMinimumSize(QSize(0, 53))   # 53 is the height of the buttons in the top row
            qf.setText('forget')
            qf.setToolTip("%s" % str(self.co['relearn_key']))
    ...
    def eventFilter(self, source, event):
        if (event.type() == QEvent.KeyPress and source is self.qrs.lineEdit):
            ...
            if self.co['secondary_accept_1']:
                sa1 = str(self.co['secondary_accept_1']).title()
                if (event.key() == eval('Qt.Key_' + sa1) and not self.qrs.lineEdit.text() == ""):
                    self.accept_read_lineedit()
                    return True
            if self.co['secondary_accept_2']:
                sa2 = str(self.co['secondary_accept_2']).title()
                if (event.key() == eval('Qt.Key_' + sa2) and not self.qrs.lineEdit.text() == ""):
                    self.accept_read_lineedit()
                    return True
    ...
    def setupHotkeys(self):
        if self.co['add_quick_buttons_to_dialog']:
            s = QShortcut(QKeySequence(str(self.co['relearn_key'])),            self, activated=lambda: self.set_and_accept (0))
